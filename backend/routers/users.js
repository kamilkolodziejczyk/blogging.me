...
router.get('/all-info/search/:id', [auth, validateObjectId], async (req, res) => {
  const user = await User.findById(req.params.id);
  if (!user) return res.status(404).send('User with this ID not exist.');
...
router.get('/followers/:id', [auth, validateObjectId], async (req, res) => {
  const users = await User.find();
  const user = await User.findById(req.params.id);
  if (!user) return res.status(404).send('User with this ID not exist.');
...
router.get('/search/:id', validateObjectId, async (req, res) => {
  const users = await User.find();
  const searchUsers = users.filter(
    user =>
      !mongoose.Types.ObjectId(user._id).equals(req.params.id)
  );
...
router.put('/unfollow/:id', [auth, validateObjectId], async (req, res) => {
  const currentUser = await User.findOne({_id: req.params.id});
  if (!currentUser) return res.status(404).send('User with this ID not exist.');
